...
| 4 | 구매 | 바로구매 → 완료 시트 |
| 5 | 로컬 연결 | 구매 완료 후 코스가 생성되며 나타남 (쇼핑 → 로컬). **구매한 상품마다 다른 코스가 제안됨** |
...
- 로컬 코스 섹션(`runLocalSequence`)도 블록을 DOM에 하나씩 붙여 실제로 자라나게 만듭니다(약 3.4초). 생성 중 표시는 항상 맨 아래로 옮겨지고, 컨테이너는 절대 위치(`scrollHeight`)로 따라 내려갑니다 — 상대 계산은 진행 중인 smooth 스크롤과 겹치면 어긋납니다.
- 선택 기준 아래에 후속 질문 3개와 답변 액션(복사 / 좋아요 / 싫어요 / 신고)이 있습니다. 데모라 클릭 시 토스트만 뜹니다.
